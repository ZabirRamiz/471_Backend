...
class property(models.Model):
    ...
    user_id = models.ForeignKey(
        user, to_field='user_id', related_name='property_owner', on_delete=models.CASCADE)

    # # had to add the related_name attribute to distinguish the foreign keys to avoid clash
    agent_id = models.ForeignKey(employee, to_field='employee_id',
                                 related_name='property_agent',  on_delete=models.CASCADE, null=True)
    support_id = models.ForeignKey(employee, to_field='employee_id',
                                   related_name='property_support', on_delete=models.CASCADE, null=True)

    market_status = models.CharField(
        default='Available For Sale', max_length=50)
